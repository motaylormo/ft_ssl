...
static const t_hash_cmd	g_hashs[] = {
	{   "md5",    ft_md5, 128},
	{  "sha1",   ft_sha1, 160},
	{"sha256", ft_sha256, 256},
	{"sha224", ft_sha224, 224},
	{"sha512", ft_sha512, 512},
	{"sha384", ft_sha384, 384},
	{(NULL), (NULL), 0}
};
...
int		run_hash(int ac, char **av)
{
	...
	for (int i = 0; g_hashs[i].str; ++i)
	{
		if (ft_strequ(av[0], g_hashs[i].str))
		{
			...
			print_checksum(env.flags, env.output_fd, hash, g_hashs[i].bits / 8);
			ft_memdel(&hash);
			free_env(env);
			return (1);
		}
...
int		run_des(int ac, char **av)
{
	...
	for (int i = 0; g_des[i].str; ++i)
	{
		if (ft_strequ(av[0], g_des[i].str))
		{
			...
			ft_pbkdf(&env);
			if (env.mode == 0)
				ft_des_encrypt(&env, g_des[i].func);
			else
				ft_des_decrypt(&env, g_des[i].func);
			free_env(env);
			return (1);
		}
	}
...
int		run_base64(int ac, char **av)
{
	...
	if (ft_strequ(av[0], "base64"))
	{
		...
		if (env.mode == 0)
			while (write_to_base64(env.output_fd, buf,
					read(env.input_fd, buf, BUFSIZE)));
		if (env.mode == 1)
			while (write(env.output_fd, buf,
					read_from_base64(env.input_fd, buf, BUFSIZE)));
		free_env(env);
		return (1);
	}
	return (0);
